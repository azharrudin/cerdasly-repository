...
<html>
    <head>
        ...
        <meta name="description" content=" <?= isset($answer["answer"]) ? substr(strip_tags($answer["answer"]),0, 300) : "Jawaban Tidak ada" ?>">        <meta name="robots" content="index, no\follow">
    ...
    ?>
        ...
        <div class="lay-container-smaller"><h5>&nbsp;Komentar:</h5></div>
        <div id="comments">
    <?php
   ...
   if($(window).height() + $(window).scrollTop()  == getDocHeight()) {
    if(!error) $.ajax({
        ...
        function commentdelete(s){
            $("#comments").animate({ opacity: 0.5 }, 100);
            $.ajax({
                url: "#",
                type: "POST",
                data: {
                ...
                success: function(data) {
                    d = data
                    $("#comments").animate({ opacity: 1 }, 100);
